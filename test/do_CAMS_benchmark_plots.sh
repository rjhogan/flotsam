...
#EXTRAS="$EXTRAS use_fitted_phase_function=1"
#echo "WARNING: USING FITTED PHASE FUNCTION FOR SINGLE SCATTERING"
...
for WAVELENGTH in 0470 1024 #0550 0660 0865
do
    if [ "$MODE" = nogasabs -o "$MODE" = nogas ]
    then
	MOLECULAR_FILE=
	...
	# values, and Cox-Munk
	for ALBEDO in 0.00 #0.10 coxm #0.05
	do
	    if [ "$ALBEDO" = "coxm" ]
	    then
